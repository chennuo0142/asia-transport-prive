...
class ArticleController extends AbstractController
{
    ...
    public function index(): Response
    {
        return $this->render('article/index.html.twig', [
            'articles' => $this->articleRepository->findBy(['user' => $this->getUser()]),
        ]);
    }
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, SluggerInterface $sluggerInterface): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $slug = strtolower($sluggerInterface->slug($article->getName() . uniqid()));

            $article->setSlug($slug)
                ->setUser($this->getUser())
                ->setCompagnyId($this->getUser()->getCompagny()->getId());
